...
-->
    ...
    <#macro renderPagingPager pageFrom pageTo pageIndex>
        <#list pageFrom..pageTo as index>
            ...
            <#else>
            <li><a href="javascript:goPaging(${index})">${index}</a></li>
            </#if>
        </#list>
    </#macro>
...
<div>
    <div class="pagination pull-right">
        <ul>
            ...
            <#if paging.hasPreviousPage()>
                <li><a href="javascript:goPaging(${(paging.pageIndex-1)})">« 上一页</a></li>
            <#else>
        ...
        -->
            ...
            <#if paging.hasNextPage()>
                <li><a href="javascript:goPaging(${(paging.pageIndex+1)})">下一页 »</a></li>
            <#else>
...
<script type="text/javascript">
    var goPaging = function (pageIndex) {
        var form = $('#${searchForm!"searchForm"}');
        var pageIndexInput = form.find("input[name=pageIndex]");
        if(!pageIndexInput || pageIndexInput.size() == 0){
            form.prepend('<input type="hidden" name="pageIndex">');
            pageIndexInput = form.find("input[name=pageIndex]");
        }
