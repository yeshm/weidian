...
-->
    ...
    <#macro renderPagingPager pageFrom pageTo pageIndex>
        <#list pageFrom..pageTo as index>
            ...
            <#else>
            <li><a href="javascript:void(0);" onclick="return goPaging(${index})">${index}</a></li>
            </#if>
        </#list>
    </#macro>
...
<div>
    <div class="pagination pull-right">
        <ul>
            ...
            <#if paging.hasPreviousPage()>
                <li><a href="javascript:void(0);" onclick="return goPaging(${(paging.pageIndex-1)})">« 上一页</a></li>
            <#else>
        ...
        -->
            ...
            <#if (total > 0) >
                <#if total lte 6>
                    <@renderPagingPager pageFrom=1 pageTo=total pageIndex=pageIndex/>
                <#else>
                    <#if pageIndex lte 4>
                        <@renderPagingPager pageFrom=1 pageTo=6 pageIndex=pageIndex/>
                    <#else>
                        <#assign tmpTotalPage = (pageIndex+2)>
                        <#if tmpTotalPage gt total>
                            <#assign tmpTotalPage = total>
                        </#if>
                        <@renderPagingPager pageFrom=(pageIndex-3) pageTo=tmpTotalPage pageIndex=pageIndex/>
                    </#if>
                </#if>
            </#if>
        ...
        -->
            ...
            <#if paging.hasNextPage()>
                <li><a href="javascript:void(0);" onclick="return goPaging(${(paging.pageIndex+1)})">下一页 »</a></li>
            <#else>
...
<script type="text/javascript">
    <#assign pagingListContainerId= (requestAttributes.pagingListContainerId)!""/>
    <#assign goPagingUrl= (requestAttributes.goPagingUrl)!""/>
    var ajaxPagingIndex = 1;
    var goPaging = function (pageIndex) {
        ajaxPagingIndex = pageIndex;
        var pagingListContainerId = '${pagingListContainerId!""}';
        var goPagingUrl = '<@ofbizUrl>${goPagingUrl!""}</@ofbizUrl>';
        var pagingPostData = app.helper.queryStringToJSON("${previousParams!""}");
        pagingPostData = $.extend(true,pagingPostData, {pageIndex: pageIndex});

        /*alert("${previousParams!""}")
        alert(${StringUtil.wrapString((previousParams)!"")});

        alert(BUI.JSON.stringify(pagingPostData));
        return;*/
        app.ajaxHelper.submitRequest({
            ...
            success: function (data) {
                if (app.ajaxHelper.handleAjaxMsg(data)) {
                    $("#" + pagingListContainerId).html(data.data.pagingListHtml)
                }
            }
        });
    }

    var refreshPaginList = function() {
        goPaging(1);
    }
