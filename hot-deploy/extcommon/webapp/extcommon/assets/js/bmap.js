function bMap(data) {
    ...
    if (data) {
        ...
        var infoWindow = new BMap.InfoWindow("原本位置 " + data.address + " ,移动红点修改位置!你也可以直接修改上方位置系统自动定位!", opts);  // 创建信息窗口对象
        marker1.openInfoWindow(infoWindow);      // 打开信息窗口
        doit(point);
    } else {
        var point = new BMap.Point(113.308837, 23.114791);
        doit(point);
    ...
    function doit(point) {
        ...
        if (point) {
            ...
            document.getElementById('geoLng').value = point.lng;
            map.setCenter(point);
            ...
            myGeo.getLocation(point, function (result) {
                if (result) {
                    document.getElementById('geoAddress').value = result.address;
                }
            });
            ...
            marker2.addEventListener("dragend", function (e) {
                document.getElementById('geoLat').value = e.point.lat;
                document.getElementById('geoLng').value = e.point.lng;
                myGeo.getLocation(new BMap.Point(e.point.lng, e.point.lat), function (result) {
                    if (result) {
                        $('geoAddress').value = result.address;
            ...
            map.addEventListener("dragend", function showInfo() {
                ...
                myGeo.getLocation(new BMap.Point(cp.lng, cp.lat), function (result) {
                    if (result) {
                        document.getElementById('geoAddress').value = result.address;
                        document.getElementById('geoLat').value = cp.lat;
                        document.getElementById('geoLng').value = cp.lng;
                        //	window.external.setaddress(result.address);//setarrea(result.address);//
                        //alert(point.lng + "  ddd " + point.lat);
    ...
    function loadmap() {
        ...
        var city = document.getElementById('geoAddress').value;
    ...
    $("#geoAddress").change(function () { loadmap(); })
    $("#geoSearchBtn").click(function () { loadmap(); });
