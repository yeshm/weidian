...
//        	Debug.logError("fieldValue is empty", module);
    ...
    public static EntityCondition createConditionFromInputFields(Delegator delegator, String entityName, Map inputFields) {
        ...
        while (it.hasNext()) {
            ...
            if (key.contains("_")) {
                ...
                //判断是否注明不根据这个字段进行过滤
                if("N".equals(inputFields.get("filterBy"+ StringUtils.capitalize(fieldName)))) continue;
                if(UtilValidate.isEmpty(fieldName) || UtilValidate.isEmpty(operation)) continue;
                entityCondition = EntityConditionWorker.createSingleCondition(delegator, entityName, fieldName, operation, inputFields.get(key));
                if (entityCondition != null) entityConditionList.add(entityCondition);
            }
        }

        if(UtilValidate.isNotEmpty(entityConditionList)){
